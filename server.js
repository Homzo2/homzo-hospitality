...
// Delete a partner account and cascade-delete their associated properties
app.delete('/api/super/partners/:id', authenticateToken, requireRole('super_admin'), (req, res) => {
  ...
  const deletedEmail = (partner.Email || '').trim().toLowerCase();
  const assignedProps = String(partner.Assigned_Properties || '')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);

  // 1. Delete associated properties and their metadata
  const properties = readExcelDb(propertiesDbPath);
  const meta = readExcelDb(partnerMetaDbPath);
  const deletedPropIds = [];

  const remainingProperties = properties.filter(p => {
    const isAssigned = assignedProps.includes(String(p.ID));
    const isOwnerEmail = (p.Email || '').trim().toLowerCase() === deletedEmail;
    if (isAssigned || isOwnerEmail) {
      deletedPropIds.push(String(p.ID));
      return false; // delete this property
    }
    return true; // keep
  });

  if (deletedPropIds.length > 0) {
    writeExcelDb(propertiesDbPath, 'Properties', remainingProperties);
    const remainingMeta = meta.filter(m => !deletedPropIds.includes(String(m.Property_ID)));
    writeExcelDb(partnerMetaDbPath, 'PartnerMeta', remainingMeta);
  }

  // 2. Remove the partner
  partners.splice(idx, 1);
  writeExcelDb(partnersDbPath, 'Partners', partners);

  // Terminate active sessions
  for (const [token, sess] of activeSessions.entries()) {
    if ((sess.email || '').trim().toLowerCase() === deletedEmail) {
      activeSessions.delete(token);
    }
  }

  logAction(req.user.email, 'super_admin', 'delete_partner', `Deleted partner ${deletedEmail} and associated properties: ${deletedPropIds.join(', ') || 'none'}`, req);
  res.json({ success: true, message: 'Partner and associated properties deleted successfully.' });
});
...
app.delete('/api/admin/properties/:id', authenticateToken, requireRole('super_admin'), (req, res) => {
  try {
    ...
    const properties = readExcelDb(propertiesDbPath);
    const idx = properties.findIndex(p => String(p.ID) === id);
    if (idx === -1) {
      return res.status(404).json({ error: 'Property not found.' });
    }
    ...
    // Also clean up PartnerMeta
    const meta = readExcelDb(partnerMetaDbPath);
    const updatedMeta = meta.filter(m => String(m.Property_ID) !== id);
    if (updatedMeta.length !== meta.length) {
      writeExcelDb(partnerMetaDbPath, 'PartnerMeta', updatedMeta);
    }

    // Also clean up Assigned_Properties in Partners
    const partners = readExcelDb(partnersDbPath);
    let partnersChanged = false;
    partners.forEach(pt => {
      const assigned = String(pt.Assigned_Properties || '').split(',').map(s => s.trim()).filter(Boolean);
      const filtered = assigned.filter(pid => String(pid) !== id);
      if (filtered.length !== assigned.length) {
        pt.Assigned_Properties = filtered.join(',');
