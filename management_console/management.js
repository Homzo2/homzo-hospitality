...
const rolePermissionsMapping = {
  'super_admin': {
    sidebar: ['dashboard', 'bookings', 'properties', 'operations', 'partners', 'guests', 'support', 'finance', 'marketing', 'reports', 'notifications', 'tasks', 'knowledge-base', 'profile', 'developer-console']
  },
  'ceo': {
    sidebar: ['dashboard', 'bookings', 'properties', 'operations', 'partners', 'guests', 'support', 'finance', 'marketing', 'reports', 'notifications', 'tasks', 'knowledge-base', 'profile', 'developer-console']
  },
  'coo': {
    sidebar: ['dashboard', 'bookings', 'properties', 'operations', 'partners', 'guests', 'support', 'finance', 'marketing', 'reports', 'notifications', 'tasks', 'knowledge-base', 'profile']
  },
  'finance head': {
    sidebar: ['dashboard', 'bookings', 'properties', 'operations', 'partners', 'guests', 'support', 'finance', 'marketing', 'reports', 'notifications', 'tasks', 'knowledge-base', 'profile']
  },
  ...
  'operations executive': {
    sidebar: ['dashboard', 'bookings', 'properties', 'operations', 'tasks', 'profile']
  },
  'customer support': {
    sidebar: ['dashboard', 'guests', 'support', 'tasks', 'profile']
  },
  'property onboarding': {
    sidebar: ['dashboard', 'properties', 'partners', 'tasks', 'profile']
  },
  'finance executive': {
    sidebar: ['dashboard', 'finance', 'reports', 'tasks', 'profile']
  },
  'marketing executive': {
    sidebar: ['dashboard', 'marketing', 'reports', 'profile']
  },
  'hr': {
    sidebar: ['dashboard', 'tasks', 'profile']
  },
  'regional manager': {
    sidebar: ['dashboard', 'bookings', 'properties', 'partners', 'guests', 'reports', 'tasks', 'profile']
  },
  'city manager': {
    sidebar: ['dashboard', 'bookings', 'properties', 'partners', 'guests', 'reports', 'tasks', 'profile']
  },
  'general admin': {
    sidebar: ['dashboard', 'bookings', 'properties', 'partners', 'guests', 'support', 'tasks', 'profile']
  },
  'developer': {
    sidebar: ['dashboard', 'developer-console', 'profile']
  }
};
  ...
  if (token && userJson) {
    try {
      ...
      if (res.ok) {
        ...
        // Prevent auto-login for company control roles on the Management Console
...
function switchPage(name) {
  ...
  if (perms && perms.sidebar && !perms.sidebar.includes(name) && name !== 'dashboard' && name !== 'settings' && name !== 'profile') {
  ...
  if (name === 'revenue' || name === 'finance') loadFinanceHub();
  if (name === 'properties') renderAdminProperties();
  if (name === 'guests') renderGuests();
  if (name === 'bookings') renderBookings();
  if (name === 'partners') loadPartners();
  if (name === 'operations') loadOperationsHub();
  if (name === 'support') loadSupportHub();
  if (name === 'marketing') loadMarketingHub();
  if (name === 'reports') loadReportsHub();
  if (name === 'notifications') loadNotificationsHub();
  if (name === 'tasks') loadTasksHub();
  if (name === 'profile') loadProfileHub();
...
if (inqSearch) {
  inqSearch.addEventListener('input', e => renderInquiries(e.target.value.toLowerCase()));
}
...
const cmpToggle = document.getElementById('compactToggle');
if (cmpToggle) {
...
const jobSch = document.getElementById('adminJobSearch');
if (jobSch) {
  jobSch.addEventListener('input', renderAdminJobs);
...
const appSch = document.getElementById('adminAppSearch');
if (appSch) {
  appSch.addEventListener('input', renderAdminApps);
...
// ════ 2. FINANCIAL CONTROL ════
function renderSacFinancial() {
  // Calculate revenue dynamically from bookings
  let totalRev = 0;
  bookingsData.forEach(b => {
    totalRev += parseFloat(String(b.amount || '0').replace(/[^0-9.]/g, '')) || 0;
  });
  
  const commission = totalRev * (sacConfig.globalComm / 100);
  const todayRev = totalRev * 0.08;
  const ytdRev = totalRev * 4.5;

  document.getElementById('sacFinTodayRev').textContent = `₹${Math.ceil(todayRev).toLocaleString()}`;
  document.getElementById('sacFinMtdRev').textContent = `₹${Math.ceil(totalRev).toLocaleString()}`;
  document.getElementById('sacFinYtdRev').textContent = `₹${Math.ceil(ytdRev).toLocaleString()}`;
  document.getElementById('sacFinCommEarned').textContent = `₹${Math.ceil(commission).toLocaleString()}`;

  // Tax Liabilities
  document.getElementById('sacFinGstLiab').textContent = `₹${Math.ceil(totalRev * 0.18).toLocaleString()}`;
  document.getElementById('sacFinTdsLiab').textContent = `₹${Math.ceil(totalRev * 0.01).toLocaleString()}`;

  // Render Payouts Table
  const tbody = document.getElementById('sacFinPayoutsTbody');
  if (mcPayouts.length === 0) {
  ...
  tbody.innerHTML = mcPayouts.map(p => {
    ...
    return `
      <tr>
        ...
        <td>${p.date}</td>
        <td><span class="sac-badge sac-badge-${p.status === 'paid' ? 'success' : (p.status === 'approved' ? 'info' : 'warning')}">${p.status.replace('_', ' ')}</span></td>
        <td>
          ${isPending ? `
            <div class="action-btns">
              <button class="act-btn success" title="Approve Payout" onclick="approveSacPayout(${p.id})"><i class="fa-solid fa-check"></i> Approve</button>
              <button class="act-btn danger" title="Hold Payout" onclick="holdSacPayout(${p.id})"><i class="fa-solid fa-circle-pause"></i> Hold</button>
            </div>
          ` : `<span style="font-size:0.75rem; color:var(--sac-text-muted);">Processed</span>`}
        </td>
      </tr>
    `;
  }).join('');
  ...
  if (p) {
    try {
      const res = await fetch(`/api/admin/payments/payout/${id}/approve`, {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({ partner: p.partner, amount: p.amount })
      });
      if (res.ok) {
        p.status = 'approved';
        showToast(`Payout request of ₹${p.amount.toLocaleString()} for ${p.partner} has been approved.`, 'success');
        renderSacFinancial();
      } else {
        showToast('Failed to approve payout on server.', 'error');
      }
    } catch (err) {
...
window.holdSacPayout = async function(id) {
  ...
  if (p) {
    try {
      ...
      if (res.ok) {
        p.status = 'held';
        showToast(`Payout request of ₹${p.amount.toLocaleString()} for ${p.partner} is placed on hold.`, 'warning');
        renderSacFinancial();
      } else {
        showToast('Failed to place payout on hold on server.', 'error');
      }
    } catch (err) {
...
async function renderMcRevenue() {
  await fetchMcPayouts();
  
  // Set all KPIs to 0
  document.getElementById('mcrKpiTotalRevenue').textContent = `₹0`;
  document.getElementById('mcrKpiCommission').textContent = `₹0`;
  ...
  document.getElementById('mcrKpiPaidOut').textContent = `₹${paidOut.toLocaleString()}`;
  document.getElementById('mcrKpiOutstanding').textContent = `₹0`;
    ...
    propTbody.innerHTML = props.map(p => {
      return `
        <tr>
          ...
          <td>₹0</td>
          <td style="color:var(--mc-gold);">₹0</td>
          <td style="color:var(--danger); font-weight:700;">₹0</td>
          <td>
            <button class="mc-btn-gold" style="padding: 4px 8px; font-size:0.75rem;" onclick="initiatePayoutMc('${p.name}', 0)">Initiate Payout</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  payoutsTbody.innerHTML = mcPayouts.map(p => {
    ...
    const breakdownHtml = `
      <div style="font-size:0.7rem; color:var(--mc-text-muted); margin-top:2px;">
  ...
  renderMcPayoutTrendChart();
}

async function initiatePayoutMc(propertyName, amount) {
  if (amount <= 0) {
    const inputAmount = prompt(`Enter custom payout amount to disburse for ${propertyName} (Min. ₹2,000):`, "5000");
    if (!inputAmount) return;
    amount = parseInt(inputAmount) || 0;
  }

  if (amount < 2000) {
    showToast('Payout request amount is below the minimum threshold limit of ₹2,000.', 'error');
    return;
  }
  ...
  try {
    const res = await fetch('/api/admin/payments/payouts', {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ partner: propertyName + ' Partner', amount })
    });
    if (res.ok) {
      showToast(`Payout request of ₹${amount.toLocaleString()} sent to Finance Team.`, 'success');
      await renderMcRevenue();
    } else {
      showToast('Failed to send payout request to server.', 'error');
    }
  } catch (err) {
    showToast('Network error initiating payout.', 'error');
  }
}

window.exportMcPayoutsCSV = function() {
...
function renderMcPayoutTrendChart() {
  const containerId = 'mcPayoutTrendChartContainer';
  let chartContainer = document.getElementById(containerId);
  if (!chartContainer) {
    chartContainer = document.createElement('div');
    chartContainer.id = containerId;
    chartContainer.style.cssText = "margin-bottom:20px; background:#18181b; border: 1px solid #27272a; border-radius:6px; padding:12px;";
    const tbody = document.getElementById('mcPayoutsTbody');
    if (tbody) {
      const card = tbody.closest('.mc-card') || tbody.parentNode.parentNode;
      if (card) {
        const title = card.querySelector('h3') || card.firstChild;
        if (title) {
  ...
  chartContainer.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
      <div style="font-size:0.78rem; color:#A1A1AA; font-weight:600;"><i class="fa-solid fa-chart-line"></i> Payouts Disbursement Trend</div>
      <button onclick="exportMcPayoutsCSV()" class="mc-btn-gold" style="padding: 2px 6px; font-size: 0.68rem; border-radius: 4px; border: 1px solid var(--mc-gold); background: transparent; color: var(--mc-gold); cursor: pointer; display: flex; align-items: center; gap: 4px;"><i class="fa-solid fa-file-csv"></i> Export CSV</button>
    </div>
    <div style="display:flex; align-items:flex-end;">
      <svg width="100%" height="${height}" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" style="overflow:visible;">
        <defs>
          <linearGradient id="mcPayoutTrendGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stop-color="var(--mc-gold)" stop-opacity="0.4"/>
            <stop offset="100%" stop-color="var(--mc-gold)" stop-opacity="0"/>
          </linearGradient>
        </defs>
        <path d="${areaD}" fill="url(#mcPayoutTrendGrad)" />
        <path d="${pathD}" fill="none" stroke="var(--mc-gold)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        ${coords.map((c, idx) => `<circle cx="${c.split(',')[0]}" cy="${c.split(',')[1]}" r="3" fill="var(--mc-gold)" stroke="#18181b" stroke-width="1" />`).join('')}
      </svg>
    </div>
  `;
}
...
window.submitNewCityStrategic = async function() {
  const name = document.getElementById('sacAddCityName').value.trim();
  const target = parseInt(document.getElementById('sacAddCityTarget').value) || 0;
  const budget = parseFloat(document.getElementById('sacAddCityBudget').value) || 0;
  const quarter = document.getElementById('sacAddCityQuarter').value.trim();
  const manager = document.getElementById('sacAddCityManager').value.trim();
  const status = document.getElementById('sacAddCityStatus').value;
  const notes = document.getElementById('sacAddCityNotes').value.trim();
  const oyo = parseInt(document.getElementById('sacAddCityOyo').value) || 0;
  const avgPrice = parseFloat(document.getElementById('sacAddCityAvgPrice').value) || 0;

  if (!name || !target || !budget || !quarter) {
...
// ─── City Dropdown Selector & Scoping Logic ───────────────────
let selectedCityFilter = 'all';

function initCitySelector() {
  const citySelect = document.getElementById('headerCitySelector');
  if (!citySelect) return;

  // Lock dropdown based on assigned city
  if (window.currentUser && window.currentUser.assignedCityId) {
    const cityId = String(window.currentUser.assignedCityId);
    if (cityId === '1') {
      selectedCityFilter = 'Mumbai';
      citySelect.value = 'Mumbai';
      citySelect.disabled = true;
    } else if (cityId === '2') {
      selectedCityFilter = 'Delhi';
      citySelect.value = 'Delhi';
      citySelect.disabled = true;
    } else if (cityId === '3') {
      selectedCityFilter = 'Bangalore';
  ...
  citySelect.addEventListener('change', (e) => {
    ...
    const activePageLink = document.querySelector('.sidebar-link.active');
    if (activePageLink) {
      switchPage(activePageLink.dataset.page);
    }
  });
}
document.addEventListener('DOMContentLoaded', initCitySelector);

// Helper function to filter datasets based on selectedCityFilter
function getFilteredData(list, propertyField = 'Property') {
  if (selectedCityFilter === 'all') return list;
  return list.filter(item => {
    const propName = (item[propertyField] || '').toLowerCase();
    // Match with property names containing city filter
    return propName.includes(selectedCityFilter.toLowerCase());
  });
}

// ─── Operations Hub Logic ─────────────────────────────────────
function loadOperationsHub() {
  // Render housekeeping
  const hkTbody = document.getElementById('housekeepingTbody');
  if (hkTbody) {
    // Mock housekeeping data
    const hkRooms = [
      { room: 'Mumbai Premier Hotel 1 - Rm 101', status: 'Clean', time: 'Today, 10:30 AM' },
      { room: 'Mumbai Premier Hotel 1 - Rm 102', status: 'Dirty', time: 'Yesterday, 04:00 PM' },
      { room: 'Delhi Executive Suites - Rm 304', status: 'Clean', time: 'Today, 11:15 AM' },
      { room: 'Bangalore Student Hub - Rm 202', status: 'Maintenance', time: '3 days ago' }
    ];
    const filteredHk = selectedCityFilter === 'all' ? hkRooms : hkRooms.filter(r => r.room.toLowerCase().includes(selectedCityFilter.toLowerCase()));
    
    hkTbody.innerHTML = filteredHk.map((r, i) => `
      <tr>
        <td>${r.room}</td>
        <td><span class="badge badge-${r.status==='Clean'?'success':r.status==='Dirty'?'danger':'warning'}">${r.status}</span></td>
        <td>${r.time}</td>
        <td><button class="btn btn-ghost btn-sm" onclick="toggleHousekeeping('${i}', '${r.status==='Clean'?'Dirty':'Clean'}')">Toggle Status</button></td>
      </tr>
    `).join('');
  }

  // Render Maintenance logs
  const maintTbody = document.getElementById('maintenanceTbody');
  if (maintTbody) {
    const maintList = [
      { prop: 'Mumbai Premier Hotel 1', issue: 'Geyser not heating in Room 104', urgency: 'High', date: 'Today, 08:15 AM', status: 'Pending' },
      { prop: 'Delhi Executive Suites', issue: 'AC cooling issue Room 201', urgency: 'Medium', date: 'Yesterday', status: 'In Progress' },
      { prop: 'Bangalore Student Hub', issue: 'Wi-Fi router replacement', urgency: 'Low', date: '2 days ago', status: 'Resolved' }
    ];
    const filteredMaint = selectedCityFilter === 'all' ? maintList : maintList.filter(m => m.prop.toLowerCase().includes(selectedCityFilter.toLowerCase()));

    maintTbody.innerHTML = filteredMaint.map((m, i) => `
      <tr>
        <td>${m.prop}</td>
        <td>${m.issue}</td>
        <td><span class="badge badge-${m.urgency==='High'?'danger':m.urgency==='Medium'?'warning':'info'}">${m.urgency}</span></td>
        <td>${m.date}</td>
        <td><span class="badge badge-${m.status==='Resolved'?'success':m.status==='Pending'?'danger':'warning'}">${m.status}</span></td>
        <td>${m.status!=='Resolved'?`<button class="btn btn-primary btn-sm" onclick="resolveMaintenance('${i}')">Resolve</button>`:'Fixed'}</td>
      </tr>
    `).join('');
  }
}

function toggleHousekeeping(idx, newStatus) {
  showToast('Housekeeping status updated to ' + newStatus, 'success');
  loadOperationsHub();
}

function resolveMaintenance(idx) {
  showToast('Maintenance task resolved!', 'success');
  loadOperationsHub();
}

function verifyCheckinOps() {
  const bkgId = document.getElementById('opsCheckinId').value.trim();
  const idOk = document.getElementById('checkinIdVerify').checked;
  const depositOk = document.getElementById('checkinDepositVerify').checked;
  const keysOk = document.getElementById('checkinKeysVerify').checked;

  if (!bkgId) {
    showToast('Please enter a valid Booking ID.', 'error');
    return;
  }
  if (!idOk || !depositOk || !keysOk) {
    showToast('Please check all verification checklist boxes.', 'error');
    return;
  }

  showToast(`Check-in completed successfully for Booking ${bkgId}!`, 'success');
  document.getElementById('opsCheckinId').value = '';
  document.getElementById('checkinIdVerify').checked = false;
  document.getElementById('checkinDepositVerify').checked = false;
  document.getElementById('checkinKeysVerify').checked = false;
}

// ─── Customer Support Hub Logic ───────────────────────────────
function loadSupportHub() {
  renderReviews();

  // Load support tickets
  const ticketsTbody = document.getElementById('supportTicketsTbody');
  if (ticketsTbody) {
    const list = [
      { guest: 'Rishabh Kumar', subject: 'Invoice issue BKG1019', status: 'Open', date: 'Today' },
      { guest: 'Surya Kumar', subject: 'WiFi connection issues', status: 'Closed', date: 'Yesterday' }
    ];
    ticketsTbody.innerHTML = list.map(t => `
      <tr>
        <td>${t.date}</td>
        <td><strong>${t.guest}</strong></td>
        <td>${t.subject}</td>
        <td><span class="badge badge-${t.status==='Open'?'danger':'success'}">${t.status}</span></td>
        <td><button class="btn btn-ghost btn-sm" onclick="showToast('Initiating ticket response...','info')">Manage</button></td>
      </tr>
    `).join('');
  }
}

function sendWhatsappSim() {
  const replyInput = document.getElementById('whatsappReplyText');
  ...
  userMsg.style.background = '#1b3152';
  userMsg.style.padding = '8px 12px';
  userMsg.style.borderRadius = '10px 10px 0 10px';
  userMsg.style.color = '#fff';
  userMsg.innerHTML = `<strong>Support:</strong> ${reply}`;

  chatWindow.appendChild(userMsg);
  chatWindow.scrollTop = chatWindow.scrollHeight;
  replyInput.value = '';
  showToast('WhatsApp response simulated successfully.', 'success');
}

// ─── Finance Hub Logic ────────────────────────────────────────
async function loadFinanceHub() {
  const currentRole = mcRole.toLowerCase();
  const allowedApprovals = ['super_admin', 'ceo', 'finance head'];

  const banner = document.getElementById('financeRestrictedBanner');
  if (banner) {
    banner.style.display = allowedApprovals.includes(currentRole) ? 'none' : 'block';
  }
  ...
  document.getElementById('finMtdRevenue').textContent = '₹' + totalRev.toLocaleString();
  document.getElementById('finTdsLiab').textContent = '₹' + Math.round(totalRev * 0.01).toLocaleString();
  document.getElementById('finPlatformComm').textContent = '₹' + Math.round(totalRev * 0.15).toLocaleString();
  document.getElementById('finGstLiab').textContent = '₹' + Math.round(totalRev * 0.18).toLocaleString();

  // Load payouts list
  const payoutsTbody = document.getElementById('finPayoutsTbody');
  if (payoutsTbody) {
    const list = [
      { id: 1, partner: 'Rajesh Mehta', property: 'Mumbai Premier Hotel 1', amount: '₹12,450', status: 'Pending' },
      { id: 2, partner: 'Suresh Kumar', property: 'Delhi Executive Suites', amount: '₹8,500', status: 'Hold' }
    ];
    
    payoutsTbody.innerHTML = list.map(p => {
      const isDisabled = !allowedApprovals.includes(currentRole);
      return `
        <tr>
          <td><strong>${p.partner}</strong></td>
          <td>${p.property}</td>
          <td style="color:var(--primary); font-weight:700;">${p.amount}</td>
          <td><span class="badge badge-${p.status==='Pending'?'warning':'danger'}">${p.status}</span></td>
          <td>
            <button class="btn btn-primary btn-sm" ${isDisabled ? 'disabled' : ''} onclick="approvePayoutMc(${p.id})">Approve</button>
            <button class="btn btn-ghost btn-sm" ${isDisabled ? 'disabled' : ''} style="color:var(--danger); margin-left:6px;" onclick="holdPayoutMc(${p.id})">Hold</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  // Load payments list
  const paymentsTbody = document.getElementById('finPaymentsTbody');
  if (paymentsTbody) {
    paymentsTbody.innerHTML = guestsData.slice(0, 8).map((g, i) => `
      <tr>
        <td><code>TXN_${10239 + g.id}</code></td>
...
function approvePayoutMc(id) {
  showToast('Payout id ' + id + ' approved and released successfully.', 'success');
  loadFinanceHub();
}

function holdPayoutMc(id) {
  showToast('Payout id ' + id + ' placed on operational hold.', 'info');
  loadFinanceHub();
}

// ─── Marketing Hub Logic ──────────────────────────────────────
const activeCoupons = [
  { code: 'WELCOME10', val: 10, type: 'percent' },
  { code: 'STUDENT500', val: 500, type: 'flat' }
];

function loadMarketingHub() {
  const tbody = document.getElementById('mktCouponsTbody');
  if (tbody) {
    tbody.innerHTML = activeCoupons.map(c => `
      <tr>
        <td><code>${c.code}</code></td>
        <td>${c.val}</td>
        <td>${c.type==='percent'?'Percentage (%)':'Flat Rate (₹)'}</td>
        <td><button class="btn btn-ghost btn-sm" style="color:var(--danger);" onclick="deletePromoCouponMc('${c.code}')"><i class="fa-solid fa-trash"></i></button></td>
      </tr>
    `).join('');
  }

  const featuredTbody = document.getElementById('mktFeaturedTbody');
  if (featuredTbody) {
    featuredTbody.innerHTML = adminProps.slice(0, 4).map(p => `
      <tr>
        <td><strong>${p.name}</strong></td>
        <td>${p.type}</td>
        <td>${p.location}</td>
        <td>15%</td>
        <td><span class="badge badge-success">Featured</span></td>
      </tr>
    `).join('');
  }
}

function addPromoCouponMc() {
  const code = document.getElementById('mktCouponCode').value.trim().toUpperCase();
  const val = parseInt(document.getElementById('mktDiscountVal').value);
  const type = document.getElementById('mktDiscountType').value;

  if (!code || isNaN(val) || val <= 0) {
    showToast('Please specify a valid code and value.', 'error');
    return;
  }

  activeCoupons.push({ code, val, type });
  showToast('Coupon code ' + code + ' created successfully!', 'success');
  document.getElementById('mktCouponCode').value = '';
  loadMarketingHub();
}

function deletePromoCouponMc(code) {
  const idx = activeCoupons.findIndex(c => c.code === code);
  if (idx > -1) {
    activeCoupons.splice(idx, 1);
    showToast('Coupon code ' + code + ' deleted.', 'info');
    loadMarketingHub();
  }
}

// ─── Reports Hub Logic ────────────────────────────────────────
let reportsRevChart = null;
let reportsRevCatChart = null;

function loadReportsHub() {
  const ctx1 = document.getElementById('mcRevChart');
  const ctx2 = document.getElementById('mcRevCatChart');
  if (!ctx1 || !ctx2) return;

  const dataCat = [45000, 32000, 24000, 15000, 18000];
  const labelsCat = ['Students', 'Employees', 'Tourists', 'Foreigners', 'Couples'];

  if (reportsRevChart) reportsRevChart.destroy();
  reportsRevChart = new Chart(ctx1, {
    type: 'line',
    data: {
      labels: ['Jan','Feb','Mar','Apr','May','Jun'],
      datasets: [{ label: 'Revenue (₹)', data: [120000, 150000, 180000, 175000, 210000, 240000], borderColor: 'var(--primary)', tension: 0.3, fill: false }]
    },
    options: { responsive: true, maintainAspectRatio: false }
  });

  if (reportsRevCatChart) reportsRevCatChart.destroy();
  reportsRevCatChart = new Chart(ctx2, {
    type: 'doughnut',
    data: {
      labels: labelsCat,
      datasets: [{ data: dataCat, backgroundColor: ['#22c55e', '#3b82f6', '#f59e0b', '#a855f7', '#ef4444'] }]
    },
    options: { responsive: true, maintainAspectRatio: false }
  });
}

// ─── Notifications Hub Logic ──────────────────────────────────
async function loadNotificationsHub() {
  const tbody = document.getElementById('notificationsLogsTbody');
  if (!tbody) return;
  ...
  try {
    ...
    if (res.ok) {
      ...
      tbody.innerHTML = logs.reverse().slice(0, 15).map(l => `
        <tr>
          <td style="white-space:nowrap; color:var(--text-muted);">${new Date(l.createdAt).toLocaleTimeString()}</td>
          <td><span class="badge badge-${l.type==='email'?'info':l.type==='sms'?'warning':'success'}">${l.type.toUpperCase()}</span></td>
          <td><code>${l.recipient}</code></td>
          <td>${l.subject || l.body.slice(0, 60) + '...'}</td>
          <td><span class="badge badge-success">Sent</span></td>
        </tr>
      `).join('');
    } else {
      tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Failed to load logs.</td></tr>';
    }
  } catch (e) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Failed to connect to logs.</td></tr>';
  }
}

// ─── Tasks & HR Hub Logic ─────────────────────────────────────
const mockTasks = [
  { id: 1, title: 'Clean room 102 kitchen area', assignee: 'OPS-102', due: '2026-08-01', status: 'Pending' },
  { id: 2, title: 'Replace shower head room 304', assignee: 'OPS-104', due: '2026-08-02', status: 'In Progress' }
];

const mockStaff = [
  { id: 'OPS-102', name: 'Karan Sharma', email: 'karan@homzo.in', role: 'Operations Executive', city: 'Mumbai', status: 'Active' },
  { id: 'OPS-104', name: 'Amit Verma', email: 'amit@homzo.in', role: 'Operations Executive', city: 'Delhi', status: 'Active' },
  { id: 'OPS-105', name: 'Sneha Patel', email: 'sneha@homzo.in', role: 'HR Manager', city: 'Bangalore', status: 'Active' }
];

function loadTasksHub() {
  // Populate Assignee options
  const select = document.getElementById('hrTaskAssignee');
  if (select) {
    select.innerHTML = mockStaff.map(s => `<option value="${s.id}">${s.name} (${s.id})</option>`).join('');
  }

  // Render tasks
  const tasksTbody = document.getElementById('hrTasksTbody');
  if (tasksTbody) {
    tasksTbody.innerHTML = mockTasks.map(t => `
      <tr>
        <td>${t.title}</td>
        <td><code>${t.assignee}</code></td>
        <td>${t.due}</td>
        <td><span class="badge badge-${t.status==='Pending'?'danger':t.status==='In Progress'?'warning':'success'}">${t.status}</span></td>
        <td>${t.status!=='Completed'?`<button class="btn btn-primary btn-sm" onclick="completeHrTask(${t.id})">Complete</button>`:'Fixed'}</td>
      </tr>
    `).join('');
  }

  // Render HR directory
  const hrTbody = document.getElementById('hrEmployeesTbody');
  if (hrTbody) {
    hrTbody.innerHTML = mockStaff.map(s => `
      <tr>
...
function dispatchHrTask() {
  const title = document.getElementById('hrTaskTitle').value.trim();
  const assignee = document.getElementById('hrTaskAssignee').value;
  const due = document.getElementById('hrTaskDueDate').value || '2026-08-05';

  if (!title) {
    showToast('Please specify a task description.', 'error');
    return;
  }

  mockTasks.push({ id: mockTasks.length + 1, title, assignee, due, status: 'Pending' });
  showToast('New operations task assigned successfully!', 'success');
  document.getElementById('hrTaskTitle').value = '';
  loadTasksHub();
}

function completeHrTask(id) {
...
function loadProfileHub() {
  if (window.currentUser) {
    document.getElementById('profileNameVal').value = window.currentUser.name || '';
    document.getElementById('profileEmailVal').value = window.currentUser.email || '';
    document.getElementById('profileRoleVal').value = window.currentUser.role || 'General Admin';
  }
}

async function updateProfilePasswordMc() {
  ...
  try {
    const email = window.currentUser ? window.currentUser.email : 'admin@homzo.in';
    const res = await fetch('/api/auth/reset-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, otp: '173691', newPassword: newPass }) // Uses simulated reset path
    });
    if (res.ok) {
